...
rimanente  = max(0, duration - trascorso)
scaduto    = trascorso >= duration
```

`max(0, ...)` impedisce che il rimanente diventi negativo quando il tempo è già esaurito —senza questo controllo la barra potrebbe avere larghezza negativa, con risultati imprevedibili.
