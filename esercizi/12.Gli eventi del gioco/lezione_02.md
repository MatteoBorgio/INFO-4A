...
Il movimento get_pressed serve a intercettare lo stato attuale della tastiera, quindi se un tasto rimane premuto. Altrimenti verrebbe intercettato solo il primo click del tasto e non se esso sta venendo premuto. 
2. Cosa restituisce `pygame.key.get_pressed()` — una lista, un dizionario, o qualcos'altro?
Restituisce una sequenza indicizzata di tuple con un indice rappresentato da una costante pygame che indica un tasto e un booleano che rappresenta se il tasto è premuto in quell'istante.
3. Se l'utente clicca fuori dalla pallina, `point_in_circle()` restituisce `False`. Cosa fa il programma in quel caso?
Non cambia lo stato della pallina, nè la posizione nè il colore.
4. Con `len(COLORS) = 5`, qual è il risultato di `(4 + 1) % 5`? E di `(3 + 1) % 5`?
Il primo risultato è 0, il secondo è 4. L'operatore modulo restituisce il resto della divisione intera.
5. Perché `clamp` viene applicato dopo tutti gli aggiornamenti della posizione e non dentro ogni `if`?
Perchè deve assicurarsi che la pallina rimanga in un intervallo (i bordi) quando la sua posizione viene aggiornata.
6. È possibile muovere la pallina in diagonale? Come?
Mantenendo premuto contemporaneamente la una tra freccia su e freccia giù e una tra freccia destra e freccia sinistra
